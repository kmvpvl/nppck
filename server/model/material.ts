...
import { IFactory, FactorySchema, IMLString } from "./factory";
...
export default class Material {
    ...
    async load(){
        ...
        .catch((err)=>{
            try {
                throw new NPPCError("mongo:connect", `err=${err.message}; materialid=${this.id}`)
            } catch(e){
                console.error(e);
            }
        });
        ...
        if (!this.data) throw new NPPCError("material:notfound", `materialid=${this.id}`);
    ...
    toJson(): IMaterial {
        if (!this.data) throw new NPPCError("material:notloaded", `materialid=${this.id}`);
        return this.data;
    }
    async getOperationsByResult(): Promise<IOperation[]> {
        ...
        .catch((err)=>{
            try {
                throw new NPPCError("mongo:connect", `err=${err.message}`)
            } catch(e){
                console.error(e);
            }
        });
    ...
    async route(count: number = 1) {
        ...
        for (const [i, op] of Object.entries(oo)) {
            ...
            if (!rcount) throw new NPPCError("material:notrouted:nocount", `id = ${this.id}; ret = ${ret}`);
            if (rcount > count) {
                stock = rcount - count;
            } else {
            ...
            for (const [x, m] of Object.entries(consofs)){
                try {
                    let mat = new Material(m.materialref);
                    await mat.load();
            ...
            ret.routes.push({
                routeoperationid: new Types.ObjectId(),
                operation: {
                    operationref: op._id,
                    name: op.name
                },
